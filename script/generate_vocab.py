...
def generate_vocab_js(vocab: dict, merges: list, unk_id: int, license_: str):
    ...
    with open(output_path, "w", encoding="utf-8") as f:
        ...
        # unk_id
        f.write(f"export const UNK_ID = {unk_id};\n\n")

        # vocab
        f.write("export const VOCAB = {\n")
        for token, tid in vocab.items():
            f.write(f"  {escape_js_quoted(token)}: {tid},\n")
        f.write("};\n\n")

        # merges
        f.write("export const MERGES = [\n")
...
def generate_vocab_d_ts(license_: str):
    ...
    with open(output_path, "w", encoding="utf-8") as f:
        ...
        f.write(f"// Auto-generated by script/generate_vocab.py — do not edit manually\n")
        f.write(f"\n")
        f.write(f"export declare const UNK_ID: number;\n")
        f.write(f"export declare const VOCAB: Record<string, number>;\n")
        f.write(f"export declare const MERGES: [string, string][];\n")
